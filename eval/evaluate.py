...
def calculate_3d_mAP(gt, predictions, iou_threshold=0.5):
...
def convert_to_lyft(results, with_score=False):
    results_lyft = []

    for file_idx, class_vals_dict in results.items():
        # print(file_idx)
        for class_name, sample_vals in class_vals_dict.items():
            for sample_val in sample_vals:
                score = 1

                if len(sample_val) == 11:
                    x1, y1, x2,  y2, tx, ty, tz, h, w, l, ry = sample_val
                else:
                    x1, y1, x2,  y2, tx, ty, tz, h, w, l, ry, score = sample_val

                pred = {
                    'sample_token': file_idx,
                    'translation': [tz, -tx, -ty],
                    'size': [w, l, h],
                    'rotation': euler_to_quaternion(0, 0, ry),
                    'name': class_name
                }
                if with_score:
                    pred['score'] = score

                results_lyft.append(pred)

    return results_lyft


def get_lyft_format_data(gt_results, det_results):
    gt_results_lyft = convert_to_lyft(gt_results, with_score=False)
    det_results_lyft = convert_to_lyft(det_results, with_score=True)
    return gt_results_lyft, det_results_lyft
