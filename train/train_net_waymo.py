...
def main():
    ...
    if cfg.DATA.VAL_DATASET_PATHS is None:
        ...
        X_train, X_val = train_test_split(train_dataset_idxs, 
                                          test_size=cfg.DATA.VAL_DATA_RATIO, 
                                          random_state=cfg.RANDOM_STATE)
        val_datasets = train_datasets
    else:
        val_dataset_paths = cfg.DATA.VAL_DATASET_PATHS
        val_datasets, val_dataset_idxs = get_sample_names(val_dataset_paths) 
        X_train, X_val = train_dataset_idxs, val_dataset_idxs
    

    logging.info(f'Number of training samples: {len(X_train)}')
    logging.info(f'Number of validation samples: {len(X_val)}')
    ...
    train_dataset = dataset_def(
        ...
        random_shift=True,
        filter_objects=True)
    logging.info('Load train dataloader')
    
    # logging.info(f'Number of elements in train sample 43:  {len(train_dataset.__getitem__(43))}')
    ...
    val_dataset = dataset_def(
        ...
        random_shift=False,
        filter_objects=True)
    logging.info('Load validation dataloader')
